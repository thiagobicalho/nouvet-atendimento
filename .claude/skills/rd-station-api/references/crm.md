...
|---|---|---|---|
| Listar | GET | `/deals` | Paginado (`page[number]`, `page[size]`, default 25) **e filtrável via RDQL** (ver abaixo) — não é só paginação, apesar de parecer pela tabela sozinha. |
| Criar | POST | `/deals` | Body `{"data": {...Deal}}`. |
| Obter | GET | `/deals/{id}` | `id` = hex 24 chars. |
| Atualizar | PUT | `/deals/{id}` | Body `{"data": {...campos a mudar}}`. |

**Filtro RDQL em `/deals`** (mesmo mecanismo de `?filter=campo:valor` já usado em `/contacts`, ver abaixo): propriedades filtráveis incluem `id`, `name` (suporta match parcial via `~`), `contact_id` ("Filtra por um ou mais IDs de contato" — é assim que se busca a negociação de um contato específico, ex. `?filter=contact_id:<rd_crm_contact_id>`), `organization_id`, `stage_id`, `pipeline_id`, `owner_id`, `campaign_id`, `lost_reason_id`, `status`, `rating`, `product_ids`, `has:product`, `created_at`/`updated_at`/`closed_at`/`expected_close_date`, e campos customizados via `@slug`. Doc oficial: `crm-v2-list-deals` e `crm-v2-rdql-filtering` (puxar com `.md` no fim da URL).
...
| Ação | Método | Path | Notas |
|---|---|---|---|
| Listar | GET | `/tasks` | Paginado, filtrável via RDQL (mesmo mecanismo de `?filter=campo:valor` de `/deals`/`/contacts`, combinando mais de uma condição com `+`, ex. `?filter=deal_id:<id>+status:open`). Propriedades filtráveis confirmadas: `deal_id` (Task de um card específico — é assim que `05 - Gerenciar Task SLA.json`, CAP-8/Story 12, encontra a Task de SLA de um deal sem listar o card inteiro), `status`, `due_date` (aceita comparadores `<`/`>`, ex. `due_date:<2026-09-04T12:00:00Z` — é assim que `06 - Lembretes e Escalonamento SLA.json` varre Tasks de SLA vencidas: `?filter=status:open+due_date:<<agora>>`). Confirmado em `crm-v2-list-tasks.md`. |
| Criar | POST | `/tasks` | Body `{"data": {...Task}}`. |
| Atualizar | PUT | `/tasks/{id}` | Body `{"data": {...campos a mudar}}`. Campos editáveis confirmados: `due_date` e `status` — `status` aceita o enum `open`\|`completed`\|`canceled` na atualização (diferente da criação, que só aceita `"open"`; **não existe valor `"done"`**). É assim que `05 - Gerenciar Task SLA.json` renova o vencimento de uma Task de SLA já aberta (`due_date` = agora + `sla_resposta_minutos`), sem tocar `status`/`name`/`deal_id`. Confirmado em `crm-v2-update-task.md`. |

Campos do `Task`: `name`, `description`, `type`, `status` (**só aceita `"open"` na criação** — não dá para criar uma Task já concluída/atrasada por este endpoint; a atualização via `PUT` aceita `open`\|`completed`\|`canceled`, sem `"done"`), `due_date`, `deal_id` (associa a Task a um card específico), `owner_ids` (array de responsáveis). `deal_id` é o vínculo relevante para o Piloto — a Task de cadastro pendente no SimplesVet e/ou alerta de duplicidade familiar (AD-11, CAP-7/Story 11) é criada com `deal_id` do card do cliente, `status="open"`, sem `type`/`due_date`/`owner_ids` (nenhum desses campos tem dado configurado/confirmado com o Nouvet ainda — mesmo padrão "não inventa" já usado em outras integrações deste projeto). A Task de acompanhamento de SLA (CAP-8/Story 12) é uma Task **distinta** no mesmo deal — `name` fixo próprio (`"Acompanhamento SLA"`), com `due_date` desde a criação, nunca reaproveita/edita a Task de cadastro pendente (a distinção entre as duas é sempre por `name`, já que ambas podem coexistir `status="open"` no mesmo `deal_id`).
